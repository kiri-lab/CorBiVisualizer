package corbivisualizer.generative;
...
public final class PerlinNoise
{
    private static final int[] permutation =
        { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 36, 103, 30, 69, 142, 8, 99, 37,
                ...
                188, 159, 86, 164, 100, 109, 198, 173, 186, 3, 64, 52, 217, 226, 250, 124, 123, 5, 202, 38, 147, 118,
                ...
                176, 115, 121, 50, 45, 127, 4, 150, 254, 138, 236, 205, 93, 222, 114, 67, 29, 24, 72, 243, 141, 128,
    ...
    static public double noise(double x, double y, double z)
    {
        ...
        double lerpU, lerpV, lerpW;
        lerpU = lerp(u, grad(p[AA], x, y, z), grad(p[BA], x - 1, y, z));
        lerpV = lerp(u, grad(p[AB], x, y - 1, z), grad(p[BB], x - 1, y - 1, z));
        lerpW = lerp(v, lerpU, lerpV);

        lerpU = lerp(u, grad(p[AA + 1], x, y, z - 1), grad(p[BA + 1], x - 1, y, z - 1));
        lerpV = lerp(u, grad(p[AB + 1], x, y - 1, z - 1), grad(p[BB + 1], x - 1, y - 1, z - 1));
        double lerpZ = lerp(v, lerpU, lerpV);

        return lerp(w, lerpW, lerpZ);
    }
